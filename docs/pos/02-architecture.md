...
### erpnext_ukrainian_integrations
Апка-конектор: privat_pos (термінал через pb-pos-gateway), monobank/privatbank/liqpay,
...
- `ukrainian_integrations` лишається апкою «зовнішні конектори» — POS звертається до неї через
  адаптери (термінал, SMS-ідентифікація), залежність **однонаправлена і опційна**
  (POS працює без встановленої integrations-апки, просто без термінала);
...
└── ua_pos/            # НОВЕ
    ...
    ├── adapters/              # fiscal.py (інтерфейс), terminal.py (інтерфейс + privat_pos impl), printing.py
